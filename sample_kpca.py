...
degree = 2
K = (np.dot(images, images.T) / trnno + 1) ** degree # Ecuacion 61 del paper
# K = (K + K.T)/2.0

# esta transformación es equivalente a centrar las imágenes originales...
unoM = np.ones([trnno, trnno]) / trnno
K = K - np.dot(unoM, K) - np.dot(K, unoM) + np.dot(unoM, np.dot(K, unoM)) # Ecuacion 49 del paper http://www.face-rec.org/algorithms/Kernel/kernelPCA_scholkopf.pdf
...
for col in range(alpha.shape[1]):
    ...
    alpha[:, col] = alpha[:, col] / np.sqrt(np.abs(lambdas[col])) # Normalizacion. Sec B.2

# pre-proyección
improypre = np.dot(K.T, alpha) # Ecuacion 17
unoML = np.ones([tstno, trnno]) / trnno
Ktest = (np.dot(imagetst, images.T) / trnno + 1) ** degree # Ecuacion 52 del paper
Ktest = Ktest - np.dot(unoML, K) - np.dot(Ktest, unoM) + np.dot(unoML, np.dot(K, unoM)) # Ecuacion 54 del paper
imtstproypre = np.dot(Ktest, alpha) # Ecuacion 51
